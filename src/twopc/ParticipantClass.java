	...
	private void initialize() {
		...
		btnCommit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					if((state.equals("INIT")||(data.trim().isEmpty()||data.equals("")))) {
						JOptionPane.showMessageDialog(null, "Can't Vote Now !");
					}
					else {
		...
		btnCommit.setBounds(200, 231, 106, 37);
			...
			public void actionPerformed(ActionEvent e) {
					...
					if(state.equals("WAITING")) {
						...
						try {
							...
							/*Building the Http Connection Request and passing Client name as body. Thus the Http Header
							are encoded around the client name data.*/
							sbconnreq.append("POST /").append("READY").append("/ HTTP/1.1\r\n").append(host).append("\r\n").
							append(userAgent).append("\r\n").append(contentType).append("\r\n").append(contentlength).append(clientname.length()).append("\r\n").
							append(date).append(new Date()).append("\r\n");
							
							dos.writeUTF(sbconnreq.toString());
							
							chatArea.append("User is Ready\n");
					...
					JOptionPane.showMessageDialog(null, "Can't do this action !");
				}
			}
		});
		btnReady.setBounds(355, 231, 106, 37);
...
*/		JLabel lblEnterParticipantName = new JLabel("Enter Participant Name");
								...
								send connection request to server*/
									...
									if(clientname.equals(null)||clientname.trim().isEmpty()||(!Pattern.matches(regex, clientname)))
									{
										...
										TimerTask tt = new TimerTask() {
											...
											@Override
											public void run() {
												if(data.equals("")&&state.equals("READY")){
													chatArea.append("Did Not get Voting Request...Local Abort!\n");
													state = "ABORT";
												}

												// else if(data.equals("")&&state.equals("READY")) {
												// 	chatArea.append("Did Not get Voting Request...Local Abort!\n");
												// 	state = "ABORT";
												// }
											}
		...
		btnConnect.setBounds(334, 7, 100, 23);
			...
			while(true) {
				...
				readChat = dis.readUTF();
					...
					if(arr[0].contains("POST")) {
						...
						if(msgin.equalsIgnoreCase("PREPARE")){
							chatArea.append("Respond to PREPARE: ");
							state = "WAITING";
							decision="";
							/*Initiating a new timer to keep track of decision of coordinator and times out 
							if it doesnt arrive in stipulated time*/
							...
							TimerTask tt2 = new TimerTask() {
								...
								public void run() {
									/*check to see whether decision has arrived or not*/
									if(!(decision.equals("g_r"))){//||decision.equals("g_a"))) {
										chatArea.append("\nParticipant not ready.. SO LOCAL ABORT !\n");
										data = "";
										state = "ABORT";
									}
								}
						...
						else if(msgin.equalsIgnoreCase("GLOBAL_READY")){
							decision="";
							chatArea.append("All Participants are ready!\n");
							// state="READY";
							data="";
						}
						
						/*In case decision is global commit*/
						else if(msgin.equalsIgnoreCase("GLOBAL_COMMIT")) {
							decision = "g_c";
