...
public class CoordinatorClass {
	...
	private HashMap<String,String> readyMap;
	...
	private Boolean isAllPrep = false;
	...
	private void initialize() {
		...
		isAllPrep = false;
		JButton btnPrep = new JButton("Prepare");
		btnPrep.addActionListener(new ActionListener(){
			...
			/*String builder object to encode the message in Http Request format*/
				...
				sbvotereq.append("POST /").append("PREPARE").append("/ HTTP/1.1\r\n").append(host).append("\r\n").
				...
				textArea.append("PREPARE message is sent\n");
				state = waiting;
				isAllPrep = false;
				...
				TimerTask tt1 = new TimerTask() {
					...
					public void run() {
						...
						int count = 0;
						for (String participant : readyMap.keySet()) {
							if(readyMap.get(participant).equals("READY")) {
								count++;
							}
							else {
								break;
						...
						if(count!=votes.size()) {
							textArea.append("Time Out. Not all participants are ready.\n");
							...
							while(!toRemove.isEmpty()) {
								...
								readyMap.remove(toRemove.remove(0));
							}
							toRemove.clear();
							
							for (String client : readyMap.keySet()) {
								readyMap.put(client, "");
							}
							state = abort;
						}
				...
				/*scheduling this check task 15 seconds*/
				t.schedule(tt1, 15000);
		...
		btnPrep.setBounds(360, 47, 140, 23);
		frmCoordinator.getContentPane().add(btnPrep);
				...
				if(state.equals("READY")){
					...
					if(msg.equals(null)||msg.trim().isEmpty()){
						...
						try {
							...
							/*COORDINATOR STATE TRANSITION FROM INIT TO WAITING*/
							textArea.append(msg+" sent for verification\n");
							state = waiting;
					...
					JOptionPane.showMessageDialog(null, "Send PREPARE message first");
				}
				
			}
		});
		btnSend.setBounds(251, 47, 75, 23);
			...
			try {
				...
				while((line = dis.readUTF())!=null) {
					...
					if(arr[0].contains("POST")) {
						...
						if(msgin.contains("CONNECTED")) {
						...
						else if(msgin.contains("READY")){
							data = msgin.split(":")[1];
							readyMap.put(data, "READY");
							textArea.append("Client: "+data+" is ready.\n");
							int counter = 0;
							for (String key : readyMap.keySet()) {
								if(readyMap.get(key).equals("READY")){
									counter++;
								}else {
									break;
								}
							...
							if(counter==(votes.size())){
								...
								textArea.append("All participants are ready\n");
								state = ready;
								
								/*String builder object to encode the message in Http Request format*/
								StringBuilder sbglobalcom = new StringBuilder();
								
								sbglobalcom.append("POST /").append("GLOBAL_READY:"+COORDINATOR_NAME).append("/ HTTP/1.1\r\n").append(host).append("\r\n").
								append(userAgent).append("\r\n").append(contentType).append("\r\n").append(contentlength).append(COORDINATOR_NAME.length()).append("\r\n").
								append(date).append(new Date()).append("\r\n");
